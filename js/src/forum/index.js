...
app.initializers.add('glowingblue-password-strength', () => {
	...
	const load = () => {
		...
		$.getScript('//cdn.jsdelivr.net/npm/zxcvbn@4.4.2/dist/zxcvbn.min.js', () => {
			isLoading = false;
			hasLoaded = true;
		});
	};
	...
	function strengthChecker(value) {
		...
		switch (result.score) {
			case 0:
			case 1:
				result.info = app.translator.trans(localePrefix + 'strengthLabels.weak');
				break;
			case 2:
			case 3:
				result.info = app.translator.trans(localePrefix + 'strengthLabels.medium');
				break;
			case 4:
				result.info = app.translator.trans(localePrefix + 'strengthLabels.strong');
				break;
		}
	...
	extend(SignUpModal.prototype, 'fields', function (items) {
		if (!this.props.token) {
			...
			const enablePasswordToggle = app.forum.attribute('psEnablePasswordToggle');
			// Colors
